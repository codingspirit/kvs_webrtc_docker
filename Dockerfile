...
RUN apt update
RUN apt upgrade -y
RUN apt install git cmake g++ pkg-config gstreamer1.0-tools libgstreamer1.0-dev libgstreamer-plugins-base1.0-dev vim -y
...
RUN curl https://raw.githubusercontent.com/codingspirit/kvs_webrtc_docker/master/cross_compile_sdk.sh >> cross_compile_sdk.sh
RUN chmod +x start_master_demo.sh
RUN chmod +x cross_compile_sdk.sh
